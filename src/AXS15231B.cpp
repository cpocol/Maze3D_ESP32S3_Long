                    ...
                    bool swapBytes)
{
    ...
    uint8_t* pDstByte1 = (uint8_t*)&qBuffer[index];
    uint8_t* pDstByte2 = pDstByte1 + 1;
    ...
    for (uint16_t j = 0; j < width; j++)
        if (swapBytes)
        {
            ...
            {
                uint8_t* pSrcByte1 = (uint8_t*)&p[width * (high - 1 - i) + j];
                uint8_t* pSrcByte2 = pSrcByte1 + 1;
            ...
            for (uint16_t i = 0; i < high; i++)
                qBuffer[index++] = (uint16_t)p[width * (high - 1 - i) + j];
...
void lcd_PushColors_mirrored_rotated_90(
                    uint16_t x,
                    uint16_t y,
                    uint16_t width,
                    uint16_t high,
                    ...
                    bool swapBytes)
{
    ...
    for (uint16_t j = 0; j < width; j++)
        if (swapBytes)
        {
            uint16_t* walker = p + width * (high - 1 - 0) + width - 1 - j;
            for (uint16_t i = 0; i < high; i++)
            {
                //uint8_t* pSrcByte1 = (uint8_t*)&p[width * (high - 1 - i) + width - 1 - j];

                *pDstByte1 = *((uint8_t*)walker + 1);
                *(pDstByte1 + 1) = *(uint8_t*)walker;
